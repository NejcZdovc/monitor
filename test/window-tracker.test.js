...
function createMockActivityStore() {
  ...
  return {
    inserts,
    updates,
    insert: jest.fn((session) => {
      const id = nextId++
      inserts.push({ id, ...session })
      return id
    }),
    update: jest.fn((id, endedAt) => {
...
function createMockCallStore() {
  ...
  return {
    ...
    update: jest.fn((id, endedAt, startedAt) => {
...
function simulatePoll(appName, windowTitle = '') {
  if (execFileCallback) {
...
// ── Import after mocks ──────────────────────────────────────────────────────
const { WindowTracker } = require('../src/main/tracking/window-tracker')

// ── Test Suite ──────────────────────────────────────────────────────────────

describe('WindowTracker', () => {
  let tracker, activityStore, callStore
  let _realDateNow
  let currentTime
